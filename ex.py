...
class BluetoothManager:
    """A class to manage Bluetooth devices using DBus."""
    ...
    def _get_managed_objects(self):
        """Gets all managed objects from Bluez."""
        if not self.bus:
    ...
    def _get_device_interface(self, device_path):
        """Gets the DBus interface for a device."""
...
class AudioSinkManager:
    """A class to manage audio sinks using pulsectl."""
    ...
    def switch_to_sink(self, sink_identifier):
        """Switches the default audio sink."""
        target_sink = None
        try:
    ...
    def find_sink_by_device_name(self, device_name):
        """Finds an audio sink by device name."""
        try:
...
def get_spotifyd_pid():
    """Gets the PID of the running spotifyd process."""
    try:
...
def switch_alsa():
    """Switches to an ALSA-compatible audio sink."""
