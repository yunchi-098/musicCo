...
class BluetoothManager:
    def __init__(self):
        ...
        try:
            ...
            logger.info("BluetoothManager started successfully.")
        except dbus.exceptions.DBusException as e:
            logger.error(f"DBus initialization error: {e}. Is the Bluetooth service running or is the adapter path correct?")
            self.bus = None
            self.adapter = None
            self.adapter_props = None
        except Exception as e:
            logger.error(f"Unexpected error while starting BluetoothManager: {e}", exc_info=True)
            self.bus = None
            self.adapter = None
            self.adapter_props = None
    ...
    def list_devices(self, discovery_duration=5):
        if not self.adapter or not self.adapter_props:
            logger.error("Bluetooth adapter was not initialized correctly.")
            return {'success': False, 'error': 'Bluetooth adapter not found or could not be initialized.', 'devices': []}
        ...
        try:
            ...
            try:
                ...
                time.sleep(discovery_duration)
                 ...
                 if "Already discovering" not in str(e):
                     logger.warning(f"Could not start discovery (maybe it was already active?): {e}")
            finally:
                 try:
            ...
            for path, interfaces in objects.items():
                if 'org.bluez.Device1' in interfaces:
                    ...
                    name = str(device_props.get('Name', device_props.get('Alias', 'Unknown Device')))
    ...
    def _get_device_interface(self, device_path):
        ...
        try:
            ...
            logger.error(f"Error while getting device interface ({device_path}): {e}", exc_info=True)
            return None
    ...
    def connect_device(self, device_path):
        ...
        if not device_interface:
            return {'success': False, 'error': f'Could not get device interface ({device_path}).'}
        ...
        try:
            ...
            if bool(props_iface.Get('org.bluez.Device1', 'Connected')):
                 ...
                 return {'success': True, 'message': f"Successfully connected to '{device_name}'."}
            else:
                 logger.error(f"Could not connect to '{device_name}' (timeout or other issue).")
                 return {'success': False, 'error': f"Could not connect to '{device_name}'."}
            ...
            logger.error(f"Error while connecting to device ({device_path}): {e}", exc_info=True)
    ...
    def disconnect_device(self, device_path):
        ...
        try:
            ...
            return {'success': True, 'message': f"Disconnected from '{device_name}'."}
    ...
    def list_sinks(self):
        ...
        try:
            with pulsectl.Pulse(self.app_name + '-list') as pulse:
                ...
                logger.info(f"{len(sinks_info)} sinks listed. Default: {default_sink_name}")
             ...
             return {'success': False, 'error': f"Audio server error: {e}", 'sinks': [], 'default_sink_name': None}
        except Exception as e:
            logger.error(f"General error while listing audio devices: {e}", exc_info=True)
            return {'success': False, 'error': f"Unknown error: {e}", 'sinks': [], 'default_sink_name': None}
            ...
            with pulsectl.Pulse(self.app_name + '-switch') as pulse:
                ...
                if isinstance(sink_identifier, int) or sink_identifier.isdigit():
                    try:
                        ...
                        if not target_sink: raise ValueError("Sink not found with index")
                    except (ValueError, pulsectl.PulseError) as e:
                         logger.error(f"Invalid or not found sink index: {sink_identifier} - Error: {e}")
                         return {'success': False, 'error': f"Invalid or not found sink index: {sink_identifier}"}
                else:
                    ...
                    for s in sinks:
                        if search_term in s.name.lower() or search_term in s.description.lower():
                            ...
                            break
                    if not found:
                        logger.error(f"Sink named/described '{sink_identifier}' not found!")
                        return {'success': False, 'error': f"Sink named/described '{sink_identifier}' not found!"}

                if not target_sink:
                     logger.error(f"Target sink could not be determined: {sink_identifier}")
                     return {'success': False, 'error': f"Target sink could not be determined: {sink_identifier}"}

                logger.info(f"Setting default sink to '{target_sink.description}' (Index: {target_sink.index})...")
                pulse.default_set(target_sink)
                logger.info(f"Default audio output successfully set to '{target_sink.description}'.")
                return {'success': True, 'message': f"Default audio output set to '{target_sink.description}'."}
            ...
            with pulsectl.Pulse(self.app_name + '-find') as pulse:
                ...
                for sink in sinks:
                    ...
                    if search_term in name_lower or search_term in desc_lower or bluez_search_term in name_lower:
                        logger.info(f"Matching sink found for '{device_name}': {sink.description}")
                        sink_info = {
                            'index': sink.index,
                            'name': sink.name,
                            'description': sink.description,
                ...
                logger.warning(f"Matching sink not found for '{device_name}'.")
                return {'success': True, 'found': False, 'sink': None}
        except pulsectl.PulseError as e:
             logger.error(f"PulseAudio/PipeWire connection error (find_sink): {e}")
             return {'success': False, 'error': f"Audio server error: {e}", 'found': False, 'sink': None}
        except Exception as e:
            logger.error(f"General error while searching for sink for device: {e}", exc_info=True)
            return {'success': False, 'error': f"Unknown error: {e}", 'found': False, 'sink': None}
...
def restart_spotifyd():
        ...
        for pid in pids:
            try:
                ...
                time.sleep(0.5)
                 ...
                 messages.append(f"PID {pid} has already terminated.")
            except Exception as e:
    ...
    try:
        ...
        if new_pid_result['success'] and new_pid_result['pids'] and any(pid not in killed_pids for pid in new_pid_result['pids']):
            ...
            messages.append("Spotifyd was terminated but could not be restarted or no new process was found.")
            start_success = False
    except FileNotFoundError:
...
def switch_alsa():
    ...
    if target_sink.get('is_default'):
        return {'success': True, 'message': f"ALSA audio output ('{target_sink.get('description')}') is already the default."}

    switch_result = audio_manager.switch_to_sink(target_sink.get('index'))
    if switch_result['success']:
         switch_result['message'] = f"Switched to ALSA audio output: {switch_result['message']}"
    else:
         switch_result['error'] = f"Could not switch to ALSA audio output: {switch_result.get('error', 'Unknown error')}"

    return switch_result

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Audio and Bluetooth Management Script")
    parser.add_argument('command', help="Command to be executed", choices=[
        'list_sinks', 'discover_bluetooth', 'pair_bluetooth',
        'disconnect_bluetooth', 'switch_to_alsa', 'set_audio_sink',
        'restart_spotifyd'
    ])
    parser.add_argument('--identifier', help="Sink index or name/description for set_audio_sink")
    parser.add_argument('--path', help="Device DBus path for pair_bluetooth and disconnect_bluetooth")
    parser.add_argument('--duration', type=int, default=5, help="Scan duration for discover_bluetooth (seconds)")
